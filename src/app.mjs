...
function sourceDetails(quote){
 ...
 const range=quote.kind==='listing_sample'?`<p><strong>Диапазон выборки: ${number(quote.low)}–${number(quote.high)} ${esc(quote.currency)} / месяц · ${quote.samples} объявлений.</strong></p><p>Выборка сформирована: ${date(quote.cohortStartedAt)}. Исключено по сроку: ${quote.excludedForTerm??0}. Срок не указан: ${quote.unknownTermCount??quote.samples}; пригодность для вашего срока нужно подтвердить.</p>`:'';
 ...
 return `<span class="price-kind ${quote.kind==='city_reference'?'reference':'direct'}">${kind}</span><details class="source-detail"><summary>Источник и состав расходов</summary><div><p>${esc(quote.note)}</p>${quote.verificationMethod==='manual'?`<p><strong>Повторная сверка — не позднее ${date(quote.reviewDueAt)}.</strong> Автоматическая загрузка пока недоступна; дата ручной сверки сохраняется.</p>`:''}${range}<p>${link(quote.sourceUrl,quote.source)} · ${esc(quote.amount)} ${esc(quote.currency)} / месяц</p><p class="muted">Страница проверена: ${date(quote.checkedAt)}${quote.sourceDate?` · Дата источника: ${date(quote.sourceDate)}`:''}</p>${quote.kind==='city_reference'?'<p class="muted">Дата страницы; даты отдельных наблюдений и размер выборки не раскрыты.</p>':''}${components}${evidence}</div></details>`;
}
